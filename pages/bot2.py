...
def export_as_pdf(messages, filename):
    try:
        ...
        pdf.set_font("Helvetica", size=12)
        for msg in messages:
            prefix = "You: " if msg["role"] == "user" else "ILLAMA: "
            content = msg["content"]
...
def export_as_text(messages, filename):
    with open(filename, 'w', encoding='utf-8') as f:
        for msg in messages:
            role = "You: " if msg["role"] == "user" else "ILLAMA: "
            f.write(f"{role}{msg['content']}\n\n")

def save_chat_history():
    with open("chat_history_gemma.json", "w") as f:
        json.dump(st.session_state.gemma_chat_history, f)

def load_chat_history():
    try:
        with open("chat_history_gemma.json", "r") as f:
            return json.load(f)
    except FileNotFoundError:
        return []

if "gemma_chat_history" not in st.session_state:
    st.session_state.gemma_chat_history = load_chat_history()

if "gemma_current_chat_id" not in st.session_state:
    st.session_state.gemma_current_chat_id = None

if "gemma_messages" not in st.session_state:
    user_name = st.session_state.get("user_name", "")
    welcome_message = f"Hi {user_name}! I'm Gemma 3, your favorite AI assistant! How can I help you today?" if user_name else "Hi I'm Gemma 3, your favorite AI assistant! How can I help you today?"
    st.session_state.gemma_messages = [{"role": "assistant", "content": welcome_message}]

if "gemma_search_term" not in st.session_state:
    st.session_state.gemma_search_term = ""

if "gemma_uploaded_image" not in st.session_state:
    st.session_state.gemma_uploaded_image = None
...
MODEL_CONFIG = {
    "model": MODEL_NAME,
    "temperature": 0.7,
    "system": "You are a helpful AI assistant powered by Gemma 3. You can engage in natural conversations and provide helpful responses."
}

    
with st.sidebar:
    st.markdown('<div class="sidebar-image">', unsafe_allow_html=True)
    st.image("static/images/gemma.png", width=150, use_container_width=False)
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown("<h3 style='text-align: center; margin-bottom: 1rem;'>💬 Chat History</h3>", unsafe_allow_html=True)
    
    
    st.session_state.gemma_search_term = st.text_input("🔍 Search chats", value=st.session_state.gemma_search_term)
    
    
    if st.button("➕ New Chat"):
        if st.session_state.gemma_messages:
            first_user_message = next((msg["content"] for msg in st.session_state.gemma_messages if msg["role"] == "user"), None)
            chat_title = first_user_message[:50] + "..." if first_user_message and len(first_user_message) > 50 else first_user_message or "New Chat"
            
            
            chat_data = {
                "id": len(st.session_state.gemma_chat_history),
                "title": chat_title,
                "messages": st.session_state.gemma_messages.copy(),
                "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M")
            }
            st.session_state.gemma_chat_history.append(chat_data)
            save_chat_history()
        
        st.session_state.gemma_messages = [{"role": "assistant", "content": "Hi I'm Gemma 3, your favorite AI assistant! How can I help you today?"}]
        st.session_state.gemma_current_chat_id = None
        st.rerun()

    if st.session_state.gemma_chat_history:
        for chat in st.session_state.gemma_chat_history:
            chat_content = " ".join([msg["content"] for msg in chat["messages"]])
            if st.session_state.gemma_search_term.lower() in chat_content.lower():
                col1, col2 = st.columns([8, 1])
                with col1:
                    if st.button(f"📝 {chat['title']} ({chat['timestamp']})", key=f"chat_{chat['id']}"):
                        st.session_state.gemma_messages = chat['messages'].copy()
                        st.session_state.gemma_current_chat_id = chat['id']
                        st.rerun()
                
                with col2:
                    if st.button("🗑️", key=f"delete_{chat['id']}"):
                        st.session_state.gemma_chat_history.remove(chat)
                        save_chat_history()
                        if st.session_state.gemma_current_chat_id == chat['id']:
                            st.session_state.gemma_current_chat_id = None
                            st.session_state.gemma_messages = [{"role": "assistant", "content": "Hi I'm Gemma 3, your favorite AI assistant! How can I help you today?"}]
                        st.rerun()
...
st.markdown("<h1 style='text-align: center;'>💬 Gemma 3 AI Assistant</h1>", unsafe_allow_html=True)

for idx, msg in enumerate(st.session_state.gemma_messages):
    message_class = "user-message" if msg["role"] == "user" else "assistant-message"
    role_prefix = "👤 You: " if msg["role"] == "user" else "🤖 Gemma 3: "
...
if prompt:
    ...
    st.session_state.gemma_messages.append(user_message)
    ...
    with st.spinner("🤖 Gemma 3 is thinking..."):
        try:
            ...
            st.session_state.gemma_messages.append({"role": "assistant", "content": full_message})
        except Exception as e:
            error_message = f"Error communicating with the model: {str(e)}"
            st.error(error_message)
            st.session_state.gemma_messages.append({"role": "assistant", "content": "I apologize, but I encountered an error processing your request. Please try again."})
            full_message = "Error: " + error_message

    st.markdown(f"""
        <div class="chat-message assistant-message">
            🤖 Gemma 3: {full_message}
        </div>
    """, unsafe_allow_html=True)

    if st.session_state.gemma_current_chat_id is not None:
        for chat in st.session_state.gemma_chat_history:
            if chat['id'] == st.session_state.gemma_current_chat_id:
                chat['messages'] = st.session_state.gemma_messages.copy()
